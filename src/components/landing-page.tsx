    ...
    <div ref={root}>
      ...
      <main id="main-content">
        ...
        <div className="closing-story">
          <section className="closing-grid" aria-label="PawFriend closing story">
            <span className="closing-rule" aria-hidden="true" />
            <article className="featured-pet" aria-labelledby="featured-title">
            <div className="featured-copy reveal">
              <h2 id="featured-title">Miso</h2>
              <dl>
                <div><dt>Age</dt><dd>4</dd></div>
                <div><dt>Breed</dt><dd>Shiba Inu</dd></div>
                <div><dt>Energy</dt><dd>Medium</dd></div>
                <div><dt>Favorite</dt><dd>Blankets</dd></div>
                <div><dt>Special talent</dt><dd>Judging you</dd></div>
              </dl>
              <p className="profile-disclaimer">{siteCopy.featured.label}</p>
              <a className="ink-link" href="#residents">{siteCopy.featured.link} <ArrowRight aria-hidden="true" /></a>
            </div>
            <Image src="/images/featured-miso.webp" alt="Miso, a dignified Shiba Inu" fill sizes="(max-width: 760px) 90vw, 34vw" />
            </article>

            <article className="mission-card" id="mission" aria-labelledby="mission-title">
            <div className="reveal">
              <h2 id="mission-title">{siteCopy.mission.title}</h2>
              <p>{siteCopy.mission.body}</p>
              <p>{siteCopy.mission.support}</p>
            </div>
            <p className="mission-note">{siteCopy.mission.note}</p>
            <Image
              className="mission-pet"
              src="/images/story/mission-otis.webp"
              alt=""
              width={520}
              height={650}
              sizes="(max-width: 840px) 72vw, 30vw"
              loading="eager"
              unoptimized
            />
            </article>

            <article className="final-cta" id="final-cta" aria-labelledby="final-title">
            <div className="final-copy reveal">
              <h2 id="final-title">{siteCopy.final.title}</h2>
              <MagneticLink href="#residents">Start your shortlist</MagneticLink>
              <p className="final-disclaimer">{siteCopy.final.note}</p>
            </div>
            <Image src="/images/final-pet-group.webp" alt="A cheerful group of rescue dogs and cats" fill sizes="(max-width: 760px) 100vw, 36vw" />
            </article>
          </section>
        </div>
      </main>
